...
public class Sabotage extends NDaysNCampaignsAgent {
	private static final String NAME = "Sabotage"; // TODO: enter a name. please remember to submit the Google form.
	...
	protected Set<NDaysAdBidBundle> getAdBids() throws AdXException {
		...
		for (Campaign c : this.getActiveCampaigns()) {
			...
			for(MarketSegment m : MarketSegment.values()) {
				...
				// if (MarketSegment.marketSegmentSubset(marketSegment, m)) {
				// 	double bid = 0.7;
				// 	SimpleBidEntry bidEntry = new SimpleBidEntry(
				// 		m, 
				// 		bid, 
				// 		c.getBudget()*bid // segment limit 
				// 	);
				// 	bids.add(bidEntry);
				// }
				if (m.equals(marketSegment)) {
					SimpleBidEntry bidEntry = new SimpleBidEntry(m,10,1000000000);
					bids.add(bidEntry);			
				}
	...
	public double getDerivative(int x, int R) {
		...
		double dER_dx = (2/a) * ((1/(1 + bot))*(a/R) - (1/(1+Math.pow(b, 2))));
